...
namespace Api_Tienda.Models.Modelos
{
    public class CategoriaModel
    {
        ...
        public RespuestaCategoria GetCategoriasAdmin()
        {
            ...
            using (var BaseDatos = new DB_TIENDAEntities())
            {
                var respuesta = (from x in BaseDatos.CATEGORIA select x).ToList();
        ...
        public RespuestaCategoria GetCategoriaById(int id)
        {
            ...
            using (var BaseDatos = new DB_TIENDAEntities())
            {
                var respuesta = (from x in BaseDatos.CATEGORIA where x.CAT_ID == id select x).FirstOrDefault();
        ...
        public RespuestaUsuario EliminarCategoria(int IdCategoria)
        {
            using (var BaseDatos = new DB_TIENDAEntities())
            {
                var respuesta = BaseDatos.SP_ELIMINAR_CATEGORIA(IdCategoria);

                if (respuesta > 0)
                {
                    respuestaUsuario.Codigo = 1;
                    respuestaUsuario.Mensaje = "Categoria eliminada correctamente";
                }
                else
                {
                    respuestaUsuario.Codigo = 0;
                    respuestaUsuario.Mensaje = "Error al eliminar categoria";
                }
